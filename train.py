...
from settings import FILE_DIR, EMBEDDING_DIM, SEQUENCE_LENGTH
...
class ChessDataset(IterableDataset):
    ...
    def __init__(self, fname: str, move_vec_file: Optional[str]=None, one_hot_file: Optional[str]=None, indices_file: Optional[str]=None, use: Literal['pretrained', 'onehot', 'indices']='indices', total: Optional[int]=None) -> None:
        """
        ...
        total : int, optional, default=None
            Total length of the game data, or `None` if unknown (used for progress bar only)
        """
        super().__init__()
        self.fname = fname
    ...
    def __iter__(self) -> Generator[Tuple[int, int, float, float, int, int, Tuple[npt.NDArray[np.int64], npt.NDArray[np.bool_]]], None, None]:
        ...
        if worker_info is None:
            ...
            split_indices = np.linspace(0, self.total, worker_info.num_workers+1, dtype=int, endpoint=True)
            start_idx = split_indices[worker_info.id]
            end_idx = split_indices[worker_info.id+1]
            with open(self.fname, 'r') as f:
...
def get_dataloaders(file_dir: str, lengths: Dict[str, Optional[int]]={}) -> Tuple[DataLoader_T, DataLoader_T, DataLoader_T]:
    """
    ...
    lengths : dict, default={}
        Dictionary with keys "train", "val", and "test" for the lengths of the training, validation, and testing data, respectively (or None for the values if not known)
    ...
    """
    train_dataloader = DataLoader(ChessDataset(os.path.join(file_dir, 'train_data.txt'), indices_file='indices.json', total=lengths.get("train", None)), batch_size=1412, pin_memory=False, num_workers=0)
    val_dataloader = DataLoader(ChessDataset(os.path.join(file_dir, 'val_data.txt'), indices_file='indices.json', total=lengths.get("val", None)), batch_size=1412, pin_memory=False, num_workers=0)
    test_dataloader = DataLoader(ChessDataset(os.path.join(file_dir, 'test_data.txt'), indices_file='indices.json', total=lengths.get("test", None)), batch_size=1412, pin_memory=False, num_workers=0)
    return train_dataloader, val_dataloader, test_dataloader

if __name__ == '__main__':
    # Hyperparameters: LR, scheduler steps, Adam betas, Adam weight decay, dropout, dim feedforward, nheads, linear output sizes
    mp.freeze_support()
    with open('lengths.json', 'r') as f:
        lengths = json.load(f)
    with open('indices.json', 'r') as f:
        number_of_moves = len(json.load(f).keys())
    ...
    if torch.cuda.device_count() > 1:
        ...
        net = model(number_of_moves).to(DEVICE)
    ...
    for epoch in range(5):
        ...
        with tqdm(total=len(train_dl), desc='Training epoch {}'.format(epoch+1)) as pbar:
            for black_elo, white_elo, time_base, time_bonus, result, checkmate, (moves, masks) in train_dl:
                ...
                if i != 0 and i % 100000 == 0:
                    ...
                    torch.save(net.state_dict(), os.path.join('checkpoints', 'checkpoint-{}.pth'.format(i//100000)))
        ...
        torch.save(net.state_dict(), 'model-epoch-{}.pth'.join(epoch+1))
